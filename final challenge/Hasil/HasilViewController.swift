...
class HasilViewController: UIViewController, NSFetchedResultsControllerDelegate, AVAudioPlayerDelegate {
    ...
    fileprivate func setupReview() {
        if valueWpm >= 200 { kontenHasil.text = "Rata-rata kecepatan bicaramu adalah \(Int(self.valueWpm)) kata per menit.\n\nKecepatan bicara ini tergolong terlalu cepat untuk presentasi pada umumnya, sebaiknya perlambat kecepatan bicara anda agar dapat membantu pendengar mengikuti alur pembicaraan dengan baik. Akan tetapi, kecepatan ini cocok untuk membawakan topik yang bertujuan mendesak perhatian pendengar."} else
            if valueWpm >= 150 { kontenHasil.text = "Rata-rata kecepatan bicaramu adalah \(Int(self.valueWpm)) kata per menit.\n\nKecepatan bicara ini tergolong cepat untuk presentasi pada umumnya, sebaiknya perlambat kecepatan ini agar dapat membantu pendengar lebih memahami inti dari pembicaraan anda dan mengikuti alur pembicaraan dengan lebih baik, akan tetapi kecepatan ini cocok digunakan untuk kalimat yang bersemangat dengan menunjukan antusiasme yang kuat."} else
                if valueWpm >= 120 { kontenHasil.text = "Rata-rata kecepatan bicaramu adalah \(Int(self.valueWpm)) kata per menit.\n\nKecepatan bicara anda sangat baik untuk presentasi pada umumnya, kecepatan ini membuat pendengar merasa sedang diajak dalam percakapan sehari-hari. Jangan lupa ubah intonasi sesuai dengan pesan yang mau anda sampaikan ya.”"} else
                    if valueWpm < 120 { kontenHasil.text = "Rata-rata kecepatan bicaramu adalah \(Int(self.valueWpm)) kata per menit.\n\nKecepatan bicara ini tergolong lambat untuk presentasi pada umumnya, sebaiknya tingkatkan lagi tempo berbicara anda lebih memicu semangat pendengar untuk tetap berada dalam alur pembicaraan anda. Ayo, jangan biarkan perhatian mereka pergi dari pembicaraanmu!"} else
